...
export default function StudentExamAuthButton({
  ...
  const { googleSignIn, logout, isGooglSigninLoading } = useFirebase();
  const router = useRouter();

  const handleSignIn = async () => {
    // Set role and redirect URL before signing in
    document.cookie = "authRole=student; path=/";
    localStorage.setItem("userRole", "student");
    localStorage.setItem(
      "postSignInRedirect",
      `/exam/${props.teacherId}/${props.examId}`
    );

    try {
      await googleSignIn("student");
      // The redirect will be handled by the Firebase hook
      router.push(`/exam/${props.teacherId}/${props.examId}`);
    } catch (error) {
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="font-medium">
          {isGooglSigninLoading ? "Signing in..." : "Continue as Student"}
        </span>
      </div>
    </Button>
  );
