...
import { GraduationCap, LogOut, Loader2, School } from "lucide-react";

export default function TeacherAuthButton() {
  const { data: session, status } = useSession();

  const handleSignIn = () => {
    document.cookie = "authRole=teacher; path=/";
    ...
    >
      <div className="flex items-center gap-2">
        <div className="w-5 h-5 bg-blue-500/20 rounded-full flex items-center justify-center group-hover:bg-blue-500/30 transition-colors">
          <School className="w-3 h-3 text-blue-600 transition-transform group-hover:scale-110" />
        </div>
